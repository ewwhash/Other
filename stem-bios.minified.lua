...
local server_api = {
  ...
  send = function(self, id, message)
    if self:isConnected() then
      local data = self.__build_package(0, id, message)
      local sent = 0
      repeat
  ...
  subscribe = function(self, id)
    if self:isConnected() then
      self.__socket.write(self.__build_package(1, id))
  ...
  unsubscribe = function(self, id)
    if self:isConnected() then
      self.__socket.write(self.__build_package(2, id))
  ...
  ping = function(self, content, timeout)
    -- send ping request
    self.__socket.write(self.__build_package(3, content))
    -- wait for response
    local time = os.time()
  ...
  __process = function(self, package)
    ...
    elseif package.type == 3 then
      self.__socket.write(self.__build_package(4, package.content))
